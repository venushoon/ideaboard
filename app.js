...
// ─────────────────────────────────────────────
// 5. Firebase 연동 및 핵심 비즈니스 로직 (인증 포함)
// ─────────────────────────────────────────────
...
const MASTER_ADMIN_EMAIL = "teacher@gmail.com"; // 마스터 관리자 이메일
...
onAuthStateChanged(auth, (user) => {
    if (user) {
        ...
        if(currentBoardId) {
            initBoardApp(); // 코드/링크로 들어온 게스트
        } else {
            // 처음 접속한 미로그인 유저 -> 로그인 화면 띄우기
...
function initLobbyApp() {
    ...
    onValue(ref(db, 'board_meta'), snap => {
        ...
        // 🌟 마스터 관리자가 아니면 자신이 만든 보드만 보이게 필터링
        const myBoards = Object.keys(data)
            .map(k => ({ id: k, ...data[k] }))
            .filter(b => b.ownerUid === currentUserUid || currentUserUid === MASTER_ADMIN_EMAIL)
